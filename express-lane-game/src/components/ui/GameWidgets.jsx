...
export const ExpProgressBar = ({ player }) => {
  ...
  const expPct = Math.min(100, (shiftsWorked / expNeeded) * 100);
  const ready = shiftsWorked >= expNeeded;
  const shiftsLeft = Math.max(0, expNeeded - shiftsWorked);
  ...
  return (
    <div className={`mt-1 text-[10px] rounded-lg px-2 py-1.5 border ${ready && meetsEdu && meetsDep && meetsItem ? 'bg-green-50 border-green-200 text-green-700' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
      <div className="flex justify-between mb-0.5">
        ...
        <span className={ready ? 'text-green-600 font-bold' : ''}>{shiftsWorked}/{expNeeded} shifts{ready ? ' ✓' : ` (${shiftsLeft} more)`}</span>
      </div>
