...
export const gameReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'END_WEEK': {
      ...
      // 8. Economy
      const { economy, economyTimer, logEntry: econLog } = advanceEconomy(s.economy, s.economyTimer);
      if (econLog) s = log(s, econLog);
      ...
      if (jonesLog) s = log(s, jonesLog);

      // Build week summary
      const wkSummary = buildWeekSummary(s.week, updatedPlayers, s.weekStartSnapshot, s.players);
